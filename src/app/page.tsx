  ...
  return (
    <main className="">
      <div className="flex flex-col items-center">
        ...
        <div>
          {files.map((file) => (
            <div key={file.name} className="flex items-center gap-2 rounded-md p-2 hover:bg-teal-100">
              {file.is_dir ? (
              ...
              <p>{file.name}</p>
              <p className="bg-rose-950 text-white pl-2 pr-2 rounded-2xl">{formatBytes(file.size)}</p>
            </div>
          ))}
        </div>
      </div>
